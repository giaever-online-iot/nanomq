...
has   "all: install command"     "$all" "sudo snap install nanomq --channel=$CH"
...
has "default snap name when SNAP_NAME unset" "$dflt" "sudo snap install nanomq --channel=$CH"
