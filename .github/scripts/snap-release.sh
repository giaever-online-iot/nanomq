...
_vercmp() {
  ...
  for i in 0 1 2; do
    x="${A[i]:-0}"; y="${B[i]:-0}"
    # nanomq upstream occasionally tags suffixed builds (0.25.2-2): truncate each
    # component at the first non-digit so the suffix is ignored for ordering.
    x="${x%%[!0-9]*}"; x="${x:-0}"
    y="${y%%[!0-9]*}"; y="${y:-0}"
...
case "$cmd" in
  ...
  channel-version)
    ...
    # So: carry the last Track token forward, find the Version field on each row, and
